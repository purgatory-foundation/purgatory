 ...
 */
contract Purgatory is IPurgatory {
    constructor () {
        deployer = msg.sender;
    ...
    uint32 public constant purgatoryTime = 24 hours;
    ...
    /// @notice Deployer address to allow withdrawing of Blast dev gas revenue shares
    ...
    function validateTransfer(address from, address operator, address recipient) public view {
        ...
        // or the collection is not enrolled. In this case msg.sender
        // is the collection
        OptStatus memory userOptStatus = optStatus[from];
        if (_isOptedOut(userOptStatus) || !enrolledCollections[msg.sender]) return;
    ...
    function _isApproved(address from, address operator, address collection) internal view returns (bool) {
        // Skip processing if user is opted out and has passed the purgatory period for the opt out
        // or the collection is not enrolled
        OptStatus memory userOptStatus = optStatus[from];
        if (_isOptedOut(userOptStatus) || !enrolledCollections[collection]) return true;
     ...
     */
    function _supportsPurgatory(IPurgatoryCollection purgatoryCollection) internal view returns (bool) {
    ...
    /** 
     * @notice Function to allow collection owners enroll additional collections under Purgatory management
     */
    function toggleCollectionEnroll(address collection) external {
        IPurgatoryCollection purgatoryCollection = IPurgatoryCollection(collection);

        if (!_supportsPurgatory(purgatoryCollection)) revert PurgatoryInterfaceNotImplemented();
        if (purgatoryCollection.owner() != msg.sender) revert Unauthorized();
    ...
    /** 
     * @notice Function to allow Purgatory deployer to claim gas revenue share from Blast
     * This is the only administrative functionality the Purgatory contract contains
     */
    function claimMyContractsGas() external {
        if (deployer != msg.sender) revert Unauthorized();
