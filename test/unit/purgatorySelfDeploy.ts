...
import { Contracts, setupIntegration, User } from "../fixtures/selfDeploy";
import { BigNumber } from "ethers";
import { Address } from "cluster";
import { ERC1155 } from "../../typechain";

describe("PurgatorySelfDeploy", function () {
  ...
  const startingTimeStamp: number = 1699419383;
  ...
  describe("PurgatorySelfDeploy core tests", async function () {
    ...
    it("forbids collection without IPurgatoryCollection interface to be enrolled in Purgatory", async function () {
      await expect(deployer.Purgatory.toggleCollectionEnroll(deployer.TestERC1155NonPurg.address)).to.be.rejectedWith("PurgatoryInterfaceNotImplemented()");
    });
    it("forbids two factor wallet approval within purgatory time", async function () {
      ...
      await expect(users[4].Purgatory.setApprovalForOperatorApproval(users[2].address, users[3].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("Unauthorized()'");
    })
    it("forbids two factor wallet denial within purgatory time", async function () {
      await users[2].TestERC1155.setApprovalForAll(users[3].address, true);
      await users[2].Purgatory.setTwoFactorWalletApprover(users[4].address, true, false, false);
      await expect(users[4].Purgatory.setApprovalForOperatorApproval(users[2].address, users[3].address, deployer.TestERC1155.address, false)).to.be.rejectedWith("Unauthorized()'");
    })
    it("forbids two factor wallet approval by unapproved wallet", async function () {
      await users[2].TestERC1155.setApprovalForAll(users[3].address, true);
      await users[2].Purgatory.setTwoFactorWalletApprover(users[4].address, true, false, false);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[3].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("Unauthorized()'");
    })
    it("forbids approved status by unapproved operator", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + (purgatoryTimeInSeconds + 10)]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("forbids set an Approved recipient of a none erolled collection", async function () {
      ...
      await deployer.Purgatory.toggleCollectionEnroll(deployer.TestERC721A.address);
      await expect(deployer.Purgatory.setApprovedRecipient(deployer.TestERC721A.address, users[2].address, true)).to.be.rejectedWith("Unauthorized()'");
    });
    ...
    it("forbids revoked two factor wallet approval to continue approving after purgatory time", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp +  (2 * (purgatoryTimeInSeconds + 10))]);
      await ethers.provider.send("evm_mine", []);
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp +  (4 * (purgatoryTimeInSeconds + 10))]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("allows admin to toggle emergency shutdown the Purgatory system", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
    })
    it("allows admin to update the purgatory time", async function () {
      const time = await deployer.Purgatory.purgatoryTime();
      expect(await deployer.Purgatory.purgatoryTime()).to.equal(time);
      await deployer.Purgatory.setPurgatoryTime(600);
      expect(await deployer.Purgatory.purgatoryTime()).to.equal(600);
    })
    it("forbids non-admin to emergency shutdown the Purgatory system", async function () {
      await expect(users[2].Purgatory.toggleEmergencyShutdown()).to.be.rejectedWith("Unauthorized()");
    })
    it("forbids non-admin to update purgatory time", async function () {
      await expect(users[2].Purgatory.setPurgatoryTime(500)).to.be.rejectedWith("Unauthorized()");
    })
    it("allows admin to permanently disable admin functionality to toggle emergency shutdown", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.disableAdminFunctionPermanently();
      expect(await deployer.Purgatory.adminFunctionPermanentlyDisabled()).to.equal(true);

      await expect(deployer.Purgatory.toggleEmergencyShutdown()).to.be.rejectedWith("AdminFunctionsPermanentlyDisabled()");
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);

      await expect(deployer.Purgatory.setPurgatoryTime(1337)).to.be.rejectedWith("AdminFunctionsPermanentlyDisabled()");
    })
    it("forbids non-admin to permanently disable admin functionality to toggle emergency shutdown", async function () {
      await expect(users[2].Purgatory.disableAdminFunctionPermanently()).to.be.rejectedWith("Unauthorized()");
      expect(await deployer.Purgatory.adminFunctionPermanentlyDisabled()).to.equal(false);
    })
    it("forbids admin from re-activating admin functionality to toggle emergency shutdown", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.disableAdminFunctionPermanently();
      expect(await deployer.Purgatory.adminFunctionPermanentlyDisabled()).to.equal(true);
    ...
    it("allows lock down mode to remain active when recently deactivated within purgatory time", async function () {
      ...
      await expect(users[2].TestERC1155.setApprovalForAll(users[3].address, true)).to.be.rejectedWith("LockDownModeEnabled()");
    ...
    it("allows lock down mode to be deactivated after purgatory time for disable is complete", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + ((purgatoryTimeInSeconds + 10) * 2)]);
      await ethers.provider.send("evm_mine", []);

      await users[2].TestERC1155.setApprovalForAll(users[3].address, true);

      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + ((purgatoryTimeInSeconds + 10) * 4)]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("forbids lock down mode to be deactivated w/ purgatory time bypass by unauthorized 2FA wallet approver", async function () {
      ...
      await expect(users[4].Purgatory.setApprovalForDeactivatingLockDown(users[2].address)).to.be.rejectedWith("Unauthorized()");
      await expect(users[2].TestERC1155.setApprovalForAll(users[3].address, true)).to.be.rejectedWith("LockDownModeEnabled()");
      expect(await (await users[2].Purgatory.approvals(deployer.TestERC1155.address, users[2].address, users[3].address)).approved).to.equal(false);
    })
    it("forbids lock down mode to be approved by 2FA wallet when being activated", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForDeactivatingLockDown(users[2].address)).to.be.rejectedWith("RequestNotFound()");
      await expect(users[2].TestERC1155.setApprovalForAll(users[3].address, true)).to.be.rejectedWith("LockDownModeEnabled()");
      expect(await (await users[2].Purgatory.approvals(deployer.TestERC1155.address, users[2].address, users[3].address)).approved).to.equal(false);
    })
    it("forbids lock down mode to be deactivated w/ purgatory time bypass by 2FA wallet approver once purgatory time is already complete", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForDeactivatingLockDown(users[2].address)).to.be.rejectedWith("RequestAlreadyCompleted()");
    })
    ...
    it("forbids transfer in lock down mode from two factor wallet", async function () {
      ...
      /// enable lock down Mode from 2FA Wallet
      await users[3].Purgatory.enableLockDownModeFromTwoFactorWalletApprover(users[2].address);
      await expect(users[2].TestERC1155.safeTransferFrom(users[2].address, users[3].address, 1, 1, "0x")).to.be.rejectedWith("LockDownModeEnabled()");
    });
    it("forbids enabling lockdown mode from two factor wallet if not authorized 2FA wallet", async function () {
      await expect(users[3].Purgatory.enableLockDownModeFromTwoFactorWalletApprover(users[2].address)).to.be.rejectedWith("Unauthorized()");
    });
    ...
    it("forbids enabling lockdown mode from two factor wallet when lockdown mode is already enabled from 2FA wallet", async function () {
      ...
      // enable it again
      await expect(users[3].Purgatory.enableLockDownModeFromTwoFactorWalletApprover(users[2].address)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");
    });
    ...
    it("allows getRemainingOperatorApprovalTime to correctly revert if short-lived approvals is not activated", async function () {
      ...
      await expect(deployer.Purgatory.getRemainingOperatorApprovalTime(deployer.TestERC1155.address, users[3].address, users[2].address)).to.be.rejectedWith("ShortLivedApprovalsNotConfigured()");
    });
    ...
    it("allows getRemainingOperatorApprovalTime to correctly return 0 as remaining time if approval is expired", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + (purgatoryTimeInSeconds + 650)]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("allows user to opt out of short-lived approval by setting to 0", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + 500000000]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("forbids user to set themselves as 2FA wallet approver", async function () {
      await expect(users[2].Purgatory.setTwoFactorWalletApprover(users[2].address, true, false, false)).to.be.rejectedWith("TwoFactorApproverSetToSelf()");
    });
    it("forbids user to set approval status to already existing status for two-factor approver", async function () {
      await users[2].Purgatory.setTwoFactorWalletApprover(users[3].address, true, false, false);
      await expect(users[2].Purgatory.setTwoFactorWalletApprover(users[3].address, true, false, false)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");

      await users[2].Purgatory.setTwoFactorWalletApprover(users[3].address, false, false, false);
      await expect(users[2].Purgatory.setTwoFactorWalletApprover(users[3].address, false, false, false)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");
    });
    it("forbids user to set approval status to already existing status for authorized recipients", async function () {
      await users[2].Purgatory.setApprovedRecipient(deployer.TestERC1155.address, users[3].address, true);
      await expect(users[2].Purgatory.setApprovedRecipient(deployer.TestERC1155.address, users[3].address, true)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");

      await users[2].Purgatory.setApprovedRecipient(deployer.TestERC1155.address, users[3].address, false);
      await expect(users[2].Purgatory.setApprovedRecipient(deployer.TestERC1155.address, users[3].address, false)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");
    });
    it("forbids user to set approval status to already existing status for global authorized recipients", async function () {
      await users[2].Purgatory.setApprovedGlobalRecipient(users[3].address, true);
      await expect(users[2].Purgatory.setApprovedGlobalRecipient(users[3].address, true)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");

      await users[2].Purgatory.setApprovedGlobalRecipient(users[3].address, false);
      await expect(users[2].Purgatory.setApprovedGlobalRecipient(users[3].address, false)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");
    });
    it("forbids user to set approval status to already existing status for opt status", async function () {
      await users[3].Purgatory.setOptStatus(true, 0);
      await expect(users[3].Purgatory.setOptStatus(true, 0)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");

      await users[3].Purgatory.setOptStatus(false, 0);
      await expect(users[3].Purgatory.setOptStatus(false, 0)).to.be.rejectedWith("ApprovalAlreadySetToSameStatus()");
    });
    it("forbids user to set approval status to already existing status for already completed operator approval", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestAlreadyCompleted()");
    });
    it("forbids user to set approval status to already existing status for already completed transfer recipient", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForTransferRecipient(users[2].address, users[3].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestAlreadyCompleted()");
    });
    it("forbids user to set approval status to already existing status for already completed global transfer recipient", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForGlobalTransferRecipient(users[2].address, users[3].address, true)).to.be.rejectedWith("RequestAlreadyCompleted()");
    });
    ...
    it("forbids 2FA wallet from approving operator unless request has been made by main wallet", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestNotFound()");
    });
    it("forbids 2FA wallet from approving transfer recipient unless request has been made by main wallet", async function () {
      ...
      // No request has been made to setApprovalForAll
      await expect(users[3].Purgatory.setApprovalForTransferRecipient(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestNotFound()");
    });
    it("forbids 2FA wallet from approving global transfer recipient unless request has been made by main wallet", async function () {
      ...
      // No request has been made to setApprovalForAll
      await expect(users[3].Purgatory.setApprovalForGlobalTransferRecipient(users[2].address, users[4].address, true)).to.be.rejectedWith("RequestNotFound()");
    });
    it("forbids short-lived approval refresh if not fully validated or already expired approval (i.e. in Purgatory time or doesn't exist)", async function () {
      ...
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC1155.address, users[4].address)).to.be.rejectedWith("RequestNotFullyCompleted()");
      // Non-existent request
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC1155.address, users[3].address)).to.be.rejectedWith("RequestNotFullyCompleted()");
    });
    it("forbids short-lived approval refresh if not expired", async function () {
      ...
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC1155.address, users[4].address)).to.be.rejectedWith("OperatorApprovalNotExpired()");
    });
    it("forbids short-lived approval refresh if not enrolled in short-lived approvals", async function () {
      ...
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC1155.address, users[4].address)).to.be.rejectedWith("ShortLivedApprovalsNotConfigured()");
    });
    it("forbids short-lived approval refresh if collection not already approved or expired", async function () {
      ...
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC1155.address, users[3].address)).to.be.rejectedWith("RequestNotFullyCompleted()");
      await expect(users[2].Purgatory.refreshApproval(deployer.TestERC721A.address, users[4].address)).to.be.rejectedWith("RequestNotFullyCompleted()");
    });
    it("forbids two factor wallet approval of non-existent request for operator approval", async function () {
      ...
      await users[2].TestERC1155.setApprovalForAll(users[3].address, true);
      await expect(users[4].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestNotFound()");
      await expect(users[4].Purgatory.setApprovalForOperatorApproval(users[2].address, users[3].address, deployer.TestERC20.address, true)).to.be.rejectedWith("RequestNotFound()");
    })
    it("forbids two factor wallet approval of non-existent request for transfer recipient approval", async function () {
      ...
      await users[2].Purgatory.setApprovedRecipient(deployer.TestERC1155.address, users[3].address, true);
      await expect(users[4].Purgatory.setApprovalForTransferRecipient(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("RequestNotFound()");
      await expect(users[4].Purgatory.setApprovalForTransferRecipient(users[2].address, users[3].address, deployer.TestERC20.address, true)).to.be.rejectedWith("RequestNotFound()");
    })
    it("forbids two factor wallet approval of non-existent request for global transfer recipient approval", async function () {
      ...
      await users[2].Purgatory.setApprovedGlobalRecipient(users[3].address, true);
      await expect(users[4].Purgatory.setApprovalForGlobalTransferRecipient(users[2].address, users[4].address, true)).to.be.rejectedWith("RequestNotFound()");
    })
    ...
    it("forbids 2FA approver from revoking already approved operators past Purgatory time from initial request", async function () {
      ...
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC1155.address, false)).to.be.rejectedWith("RequestAlreadyCompleted()");
      expect(await (await users[2].Purgatory.operatorApprovalStatus(deployer.TestERC1155.address, users[2].address, users[4].address)).approvalStatus).to.equal(0);
    })
    it("forbids 2FA approver from revoking already approved transfers past Purgatory time from initial request", async function () {
      ...
      expect(await (await users[2].Purgatory.transferRecipientApprovalStatus(deployer.TestERC1155.address, users[2].address, users[4].address)).approvalStatus).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForTransferRecipient(users[2].address, users[4].address, deployer.TestERC1155.address, false)).to.be.rejectedWith("RequestAlreadyCompleted()");
      expect(await (await users[2].Purgatory.transferRecipientApprovalStatus(deployer.TestERC1155.address, users[2].address, users[4].address)).approvalStatus).to.equal(0);
    })
    it("forbids 2FA approver from revoking already approved global transfers past Purgatory time from initial request", async function () {
      ...
      expect(await (await users[2].Purgatory.transferRecipientApprovalStatus(deployer.TestERC1155.address, users[2].address, users[4].address)).approvalStatus).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForGlobalTransferRecipient(users[2].address, users[4].address, false)).to.be.rejectedWith("RequestAlreadyCompleted()");
      expect(await (await users[2].Purgatory.transferRecipientApprovalStatus(deployer.TestERC1155.address, users[2].address, users[4].address)).approvalStatus).to.equal(0);
    })
    ...
    it("allows admin to add and remove additional admins", async function () {
      await deployer.Purgatory.toggleAuthorizedAdmin(users[2].address);
      await users[2].Purgatory.toggleEmergencyShutdown();

      await deployer.Purgatory.toggleAuthorizedAdmin(users[2].address);
      await expect(users[2].Purgatory.toggleEmergencyShutdown()).to.be.rejectedWith("Unauthorized()");
    })
    it("forbids admins from revoking their own access", async function () {
      await expect(deployer.Purgatory.toggleAuthorizedAdmin(deployer.address)).to.be.rejectedWith("InvalidAdminAddress()");
    })
  });

  // ------------------------------------- PurgatorySelfDeploy ERC1155 ---------------------------------------

  describe("PurgatorySelfDeploy ERC1155 tests", async function () {
    ...
    it("forbids transfer to recipient within purgatory time", async function () {
      ...
      await expect(users[2].TestERC1155.safeTransferFrom(users[2].address, users[3].address, 1, 1, "0x")).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    ...
    it("forbids bypassing with unapproved two factor wallet approver by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked approvers", async function () {
      ...
      expect(await deployer.TestERC1155.balanceOf(users[3].address, 1)).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC1155.address, true)).to.be.rejectedWith("Unauthorized()");
      expect(await deployer.TestERC1155.balanceOf(users[3].address, 1)).to.equal(0);
    })
    ...
    it("allows approval to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
      ...
      await users[2].TestERC1155.setApprovalForAll(users[4].address, true);
      expect(await deployer.TestERC1155.balanceOf(users[3].address, 1)).to.equal(0);
      await users[4].TestERC1155.safeTransferFrom(users[2].address, users[3].address, 1, 1, "0x");
      expect(await deployer.TestERC1155.balanceOf(users[3].address, 1)).to.equal(1);
    })
    it("allows transfer recipient to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
    ...
    it("allows approval to correctly fail while in purgatory time if emergency shutdown was enabled and then disabled", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
    ...
    it("allows transfer recipient to correctly fail while in purgatory time if emergency shutdown was enabled and then disabled", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
    ...
    it("allows temporary approval to be valid during approval length", async function () {
      ...
      const purgatoryTimeInSeconds = await deployer.Purgatory.purgatoryTime();
      let newTime = startingTimeStamp + (purgatoryTimeInSeconds + 10);
    ...
    it("forbids transfer to 2FA wallet NOT marked as approved recipient", async function () {
      ...
      await expect(users[2].TestERC1155.safeTransferFrom(users[2].address, users[4].address, 1, 1, "0x")).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    ...
    it("forbids non-deployer to update the Purgatory address", async function () {
      await expect(users[2].TestERC1155.setPurgatoryAddress(users[2].address)).to.be.revertedWithCustomError(deployer.TestERC1155, "Unauthorized");
    })
    ...
    it("allows burning of tokens to not be impacted by Purgatory checks", async function () {
      ...
      await expect(users[2].TestERC1155Burnable.burn(users[2].address, 1, 1)).to.not.be.rejectedWith("UnauthorizedTransferRecipient()");
    ...
    it("forbids burning of tokens when in lockdown mode", async function () {
      ...
      await expect(users[2].TestERC1155Burnable.burn(users[2].address, 1, 1)).to.be.rejectedWith("LockDownModeEnabled()");
      expect(await deployer.TestERC1155Burnable.balanceOf(users[2].address, 1)).to.equal(1);
    })
    ...
    it("requires re-approval for approval that was set when system was in emergency shutdown mode", async function () {
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("forbids approval from being valid after short-lived approval has expired", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + (purgatoryTimeInSeconds + 10 + 500)]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("allows setApprovalForAll from being called again if approval is expired without throwing AlreadyApproved() error", async function () {
      ...
      expect(await users[2].TestERC1155.isApprovedForAll(users[2].address, users[3].address)).to.equal(false);
      await expect(users[2].TestERC1155.setApprovalForAll(users[3].address, true)).to.not.be.rejectedWith("AlreadyApproved()");
    });
    it("forbids expired short-lived approvals renew until Purgatory state is complete when short-lived approvals are deactivated", async function () {
      ...
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp + ((purgatoryTimeInSeconds + 310) * 2)]);
      await ethers.provider.send("evm_mine", []);
  ...
  // ------------------------------------- PurgatorySelfDeploy ERC721 ---------------------------------------

  describe("PurgatorySelfDeploy ERC721 tests", async function () {
    it("allows minting of tokens without Purgatory blocking/reverting", async function () {
    ...
    it("forbids transfer by approved operator via setApprovalForAll within purgatory time", async function () {
      ...
      await expect(users[3].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1)).to.be.revertedWithCustomError(contracts.TestERC721A, "TransferCallerNotOwnerNorApproved");
    })
    it("forbids transfer by approved operator via approve within purgatory time", async function () {
      ...
      await expect(users[3].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1)).to.be.rejectedWith('UnauthorizedOperatorApproval()');
    })
    ...
    it("allows instant removal of approval status (without purgatory time) when setApprovalForAll is revoked", async function () {
      ...
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(1);
      await expect(users[3].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 2)).to.be.revertedWithCustomError(contracts.TestERC721A, "TransferCallerNotOwnerNorApproved");
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(1);
    })
    ...
    it("forbids transfer to recipient within purgatory time", async function () {
      ...
      await expect(users[2].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1)).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    it("forbids bypassing with unapproved recipient by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked recipients", async function () {
      await deployer.TestERC721A.mint(2, users[2].address);
      await users[2].Purgatory.setApprovedRecipient(deployer.TestERC721A.address, users[3].address, true);
    ...
    it("forbids bypassing with unapproved operator by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked operators", async function () {
      ...
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
      await expect(users[4].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1)).to.be.revertedWithCustomError(contracts.TestERC721A, "TransferCallerNotOwnerNorApproved");
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
    })
    it("forbids bypassing with unapproved two factor wallet approver by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked approvers", async function () {
      await deployer.TestERC721A.mint(2, users[2].address);
      ...
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC721A.address, true)).to.be.rejectedWith("Unauthorized()");
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
    })
    it("forbids bypassing with unapproved two factor wallet approver by double revoking an approved approver to bypass griefing protection for 0 timestamp on recently revoked approvers", async function () {
      await deployer.TestERC721A.mint(2, users[2].address);
      ...
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC721A.address, true)).to.be.rejectedWith("RequestNotFound()");
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
    })
    it("allows approval via setApprovalForAll to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);
      ...
      await users[2].TestERC721A.setApprovalForAll(users[4].address, true);
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
      await users[4].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1);
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(1);
    })
    it("allows approval via approve to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      await deployer.Purgatory.toggleEmergencyShutdown();
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(true);

      await deployer.TestERC721A.mint(2, users[2].address);
      await users[2].TestERC721A.approve(users[4].address, 1);
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(0);
      await users[4].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1);
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(1);
    })
    ...
    it("forbids approval in lock down mode", async function () {
      await users[2].Purgatory.toggleLockDownMode();
      await expect(users[2].TestERC721A.setApprovalForAll(users[3].address, true)).to.be.rejectedWith("LockDownModeEnabled()");
    })
    it("forbids transfer in lock down mode", async function () {
      await deployer.TestERC721A.mint(2, users[2].address);
      await users[2].Purgatory.toggleLockDownMode();
      await expect(users[2].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[3].address, 1)).to.be.rejectedWith("LockDownModeEnabled()");
    })
    it("forbids isApprovedForAll from being true when in lock down mode", async function () {
      await ethers.provider.send("evm_setNextBlockTimestamp", [startingTimeStamp]);
      await ethers.provider.send("evm_mine", []);
    ...
    it("forbids transfer to 2FA wallet NOT marked as approved recipient", async function () {
      ...
      await expect(users[2].TestERC721A["safeTransferFrom(address,address,uint256)"](users[2].address, users[4].address, 1)).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    it("forbids transfer to 2FA wallet marked as approved recipient but without completed Purgatory state", async function () {
      await deployer.TestERC721A.mint(1, users[2].address);
      await users[2].Purgatory.setTwoFactorWalletApprover(users[4].address, true, false, true);
    ...
    it("forbids non-deployer to update the Purgatory address", async function () {
      await expect(users[2].TestERC721A.setPurgatoryAddress(users[2].address)).to.be.revertedWithCustomError(deployer.TestERC721A, "Unauthorized");
    })
    it("forbids deployer from update the Purgatory address after locking", async function () {
    ...
    it("allows burning of tokens to not be impacted by Purgatory checks", async function () {
      ...
      await expect(users[2].TestERC721ABurnable.burn(1)).to.not.be.rejectedWith("UnauthorizedTransferRecipient()")
    ...
    it("forbids burning of tokens when in lockdown mode", async function () {
      ...
      await expect(users[2].TestERC721ABurnable.burn(1)).to.be.rejectedWith("LockDownModeEnabled()")
      expect(await deployer.TestERC721ABurnable.balanceOf(users[2].address)).to.equal(1);
    })
    it("allows minting of tokens when in lockdown mode", async function () {
      await users[2].Purgatory.toggleLockDownMode();
    ...
    it("requires re-approval for approval that was set when user was opted out", async function () {
      ...
      await expect(users[2].TestERC721A["safeTransferFrom(address,address,uint256)"](users[3].address, users[2].address, 2)).to.be.revertedWithCustomError(contracts.TestERC721A, "TransferCallerNotOwnerNorApproved");
      expect(await deployer.TestERC721A.balanceOf(users[3].address)).to.equal(1);
    ...
    it("allows setApprovalForAll from being called again if approval is expired without throwing AlreadyApproved() error", async function () {
      ...
      expect(await users[2].TestERC721A.isApprovedForAll(users[2].address, users[3].address)).to.equal(false);
      await expect(users[2].TestERC721A.setApprovalForAll(users[3].address, true)).to.not.be.rejectedWith("AlreadyApproved()");
    });
    it("forbids expired short-lived approvals renew until Purgatory state is complete when short-lived approvals are deactivated", async function () {
      await deployer.TestERC721A.mint(2, users[2].address);
      const purgatoryTimeInSeconds = await deployer.Purgatory.purgatoryTime();
  ...
  // ------------------------------------- PurgatorySelfDeploy ERC20 ---------------------------------------

  describe("PurgatorySelfDeploy ERC20 tests", async function () {
    it("allows minting of tokens without Purgatory blocking/reverting", async function () {
    ...
    it("forbids transfer to recipient within purgatory time", async function () {
      ...
      await expect(users[2].TestERC20.transfer(users[3].address, 1)).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    it("forbids bypassing with unapproved recipient by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked recipients", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
      await users[2].Purgatory.setApprovedRecipient(deployer.TestERC20.address, users[3].address, true);
    ...
    it("forbids bypassing with unapproved two factor wallet approver by setting then revoking to bypass griefing protection for 0 timestamp on recently revoked approvers", async function () {
      ...
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC20.address, true)).to.be.rejectedWith("Unauthorized()");
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(0);
    })
    it("forbids bypassing with unapproved two factor wallet approver by double revoking an approved approver to bypass griefing protection for 0 timestamp on recently revoked approvers", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
      ...
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(0);
      await expect(users[3].Purgatory.setApprovalForOperatorApproval(users[2].address, users[4].address, deployer.TestERC20.address, true)).to.be.rejectedWith("RequestNotFound()");
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(0);
    })
    it("allows approval via setApprovalForAll to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
      ...
      await users[2].TestERC20.approve(users[4].address, 10000000);
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(0);
      await users[4].TestERC20.transferFrom(users[2].address, users[3].address, 1);
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(1);
    })
    it("allows approval via approve to fail open (as a normal NFT contract) when emergency shutdown is active", async function () {
      expect(await deployer.Purgatory.emergencyShutdownActive()).to.equal(false);
    ...
    it("forbids approval in lock down mode", async function () {
      await users[2].Purgatory.toggleLockDownMode();
      await expect(users[2].TestERC20.approve(users[3].address, 1000000000)).to.be.rejectedWith("LockDownModeEnabled()");
    })
    it("forbids transfer in lock down mode", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
      await users[2].Purgatory.toggleLockDownMode();
      await expect(users[2].TestERC20.transfer(users[3].address, 1)).to.be.rejectedWith("LockDownModeEnabled()");
    })
    ...
    it("forbids transfer to 2FA wallet NOT marked as approved recipient", async function () {
      ...
      await expect(users[2].TestERC20.transfer(users[4].address, 1)).to.be.rejectedWith("UnauthorizedTransferRecipient()");
    })
    it("forbids transfer to 2FA wallet marked as approved recipient but without completed Purgatory state", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
      await users[2].Purgatory.setTwoFactorWalletApprover(users[4].address, true, false, true);
    ...
    it("forbids non-deployer to update the Purgatory address", async function () {
      await expect(users[2].TestERC20.setPurgatoryAddress(users[2].address)).to.be.revertedWithCustomError(deployer.TestERC20, "Unauthorized");
    })
    it("forbids deployer from update the Purgatory address after locking", async function () {
    ...
    it("allows burning of tokens to not be impacted by Purgatory checks", async function () {
      ...
      await expect(users[2].TestERC20Burnable.burn(10000000)).to.not.be.rejectedWith("UnauthorizedTransferRecipient()")
    ...
    it("forbids burning of tokens when in lockdown mode", async function () {
      ...
      await expect(users[2].TestERC20Burnable.burn(10000000)).to.be.rejectedWith("LockDownModeEnabled()")
      expect(await deployer.TestERC20Burnable.balanceOf(users[2].address)).to.equal(10000000);
    })
    it("allows minting of tokens when in lockdown mode", async function () {
      await users[2].Purgatory.toggleLockDownMode();
    ...
    it("requires re-approval for approval that was set when system was in emergency shutdown mode", async function () {
      ...
      await await users[3].TestERC20.transferFrom(users[2].address, users[3].address, 1);
      expect(await deployer.TestERC20.balanceOf(users[3].address)).to.equal(2);
    })
    it("allowances approve to be called multiple times without AlreadyApproved() error", async function () {
      ...
      expect(await users[2].TestERC20.allowance(users[2].address, users[3].address)).to.not.equal(0);
      await expect(users[2].TestERC20.approve(users[3].address, 500000)).to.not.be.rejectedWith("AlreadyApproved()");
    })
    it("forbids increaseAllowance from being called", async function () {
      await expect(users[2].TestERC20.increaseAllowance(users[3].address, 500000)).to.be.revertedWithCustomError(contracts.TestERC20, "FunctionNotSupported");
    })
    it("forbids decreaseAllowanceAllowance from being called", async function () {
      await expect(users[2].TestERC20.decreaseAllowance(users[3].address, 500000)).to.be.revertedWithCustomError(contracts.TestERC20, "FunctionNotSupported");
    })
    it("forbids approval from being valid after short-lived approval has expired", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
    ...
    it("allows setApprovalForAll from being called again if approval is expired without throwing AlreadyApproved() error", async function () {
      ...
      expect(await users[2].TestERC20.allowance(users[2].address, users[3].address)).to.equal(0);
      await expect(users[2].TestERC20.approve(users[3].address, 1000000000)).to.not.be.rejectedWith("AlreadyApproved()");
    });
    it("forbids expired short-lived approvals renew until Purgatory state is complete when short-lived approvals are deactivated", async function () {
      await deployer.TestERC20.mint(10000000, users[2].address);
      const purgatoryTimeInSeconds = await deployer.Purgatory.purgatoryTime();
